...
// 最终清理导入问题
function finalCleanupImports(content, filePath) {
  let modified = false;
  let newContent = content;
  
  // 检查是否需要 Badge 组件
  if (/<Badge[\s>]/.test(newContent)) {
    const antdImportRegex = /import\s*{([^}]*)}\s*from\s*['"]antd['"]/;
    const match = newContent.match(antdImportRegex);
    
    if (match && !match[1].includes('Badge')) {
      ...
      modified = true;
      console.log(`${filePath}: 添加 Badge 导入`);
    }
  }
  
  // 移除未使用的组件导入
  const unusedComponents = ['message', 'Modal'];
    ...
    if (!isComponentUsed(newContent, component)) {
      ...
      if (match && match[1].includes(component)) {
        const components = match[1]
          .split(',')
          .map(c => c.trim())
          .filter(c => c.length > 0 && c !== component);
        ...
        modified = true;
        console.log(`${filePath}: 移除未使用的 ${component} 导入`);
      }
    }
  }
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        ...
        try {
          ...
          const result = finalCleanupImports(content, fullPath);
...
function main() {
  ...
  console.log('开始最终清理导入问题...');
  const fixedCount = processDirectory(srcPath);
  console.log(`\n最终清理完成！共处理了 ${fixedCount} 个文件`);
}
