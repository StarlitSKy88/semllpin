...
export function Scene() {
  ...
  const { particles, originalPositionTexture, particlePositions } = useMemo(() => {
    ...
    // Adjust geometry size: reduce overall size by 30%, extend horizontally by 30%
    const geometry = new THREE.TorusKnotGeometry(1.26, 0.315, 400, 32)
    geometry.scale(1.3, 1.0, 1.0) // Extend horizontally by 30%
  ...
  useFrame((state) => {
    ...
    if (pointsRef.current) {
      // Increase rotation speed 12%
      pointsRef.current.rotation.y += 0.00112
      pointsRef.current.rotation.x += 0.00056
    }
  })
