...
export class PayPalService {
  ...
  static async createOrder(request: PayPalCreateOrderRequest): Promise<PayPalOrderResponse> {
    ...
    const paymentData = {
      ...
      redirect_urls: {
        return_url: `${process.env['APP_BASE_URL'] || 'http://localhost:3000'}/api/payments/success`,
        cancel_url: `${process.env['APP_BASE_URL'] || 'http://localhost:3000'}/api/payments/cancel`
      }
    };
