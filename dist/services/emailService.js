    ...
    initializeTransporter() {
        try {
            ...
            if (!emailConfig.auth.user || !emailConfig.auth.pass) {
                logger_1.logger.warn('邮件服务未配置，将跳过邮件发送');
                return;
            }
            ...
            this.transporter?.verify((error) => {
                if (error) {
                    logger_1.logger.error('邮件服务配置验证失败:', error);
                    this.isConfigured = false;
                }
                else {
                    logger_1.logger.info('邮件服务配置验证成功');
                }
            });
        }
        catch (error) {
            logger_1.logger.error('初始化邮件服务失败:', error);
        }
    }
...
取消订阅: ${baseUrl}/unsubscribe
    ...
    async sendNotificationEmail(to, notificationData) {
        if (!this.isConfigured || !this.transporter) {
            logger_1.logger.warn('邮件服务未配置，跳过邮件发送');
            return false;
        }
        try {
            ...
            logger_1.logger.info(`通知邮件发送成功: ${result.messageId}`);
            return true;
        }
        catch (error) {
            logger_1.logger.error('发送通知邮件失败:', error);
            return false;
        }
    }
    async sendWelcomeEmail(to, username) {
        ...
        `,
            ...
            const result = await this.transporter.sendMail(mailOptions);
            logger_1.logger.info(`欢迎邮件发送成功: ${result.messageId}`);
            return true;
        }
        catch (error) {
            logger_1.logger.error('发送欢迎邮件失败:', error);
            return false;
        }
    }
    async testEmailService() {
        ...
        try {
            ...
            logger_1.logger.error('邮件服务测试失败:', error);
            return false;
        }
    }
}
