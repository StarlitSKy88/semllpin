...
exports.simulator = exports.MultiAgentSimulator = void 0;
const faker_1 = require("@faker-js/faker");
const axios_1 = __importDefault(require("axios"));
const events_1 = require("events");
const fs = __importStar(require("fs/promises"));
const path = __importStar(require("path"));
class UserAgent extends events_1.EventEmitter {
    constructor(config) {
        super();
        ...
        this.httpClient = axios_1.default.create({
            ...
            headers: {
                ...
                'User-Agent': `SmellPin-Agent-${config.id}/${faker_1.faker.system.semver()}`
            }
        });
        this.metrics = {
            agentId: config.id,
    ...
    async initializeUser() {
        ...
        try {
            this.userProfile = {
                ...
                location: {
                    lat: faker_1.faker.location.latitude({ min: 39.8, max: 40.2 }),
                    lng: faker_1.faker.location.longitude({ min: 116.2, max: 116.6 })
                }
            };
    ...
    async executeBehaviorPattern() {
        ...
        while (this.isRunning && Date.now() < endTime) {
            try {
                ...
                const waitTime = faker_1.faker.number.int({ min: 1000 / intensityMultiplier, max: 5000 / intensityMultiplier });
    ...
    async explorerBehavior() {
        ...
        const randomAction = faker_1.faker.helpers.arrayElement(actions);
        await randomAction();
    }
    async annotatorBehavior() {
        const actions = [
    ...
    async searchNearbyAnnotations() {
        ...
        try {
            const params = {
                lat: this.userProfile.location.lat + faker_1.faker.number.float({ min: -0.01, max: 0.01 }),
                lng: this.userProfile.location.lng + faker_1.faker.number.float({ min: -0.01, max: 0.01 }),
    ...
    async createAnnotation() {
        ...
        try {
            const annotationData = {
                ...
                location: {
                    lat: this.userProfile.location.lat + faker_1.faker.number.float({ min: -0.005, max: 0.005 }),
                    lng: this.userProfile.location.lng + faker_1.faker.number.float({ min: -0.005, max: 0.005 })
                },
                category: faker_1.faker.helpers.arrayElement(['industrial', 'sewage', 'garbage', 'chemical', 'food', 'other']),
                intensity: faker_1.faker.number.int({ min: 1, max: 10 }),
                tags: faker_1.faker.helpers.arrayElements(['臭味', '污染', '环境', '工业'], { min: 1, max: 3 })
            };
    ...
    async viewAnnotationDetails() {
        const action = this.createAction('view_annotation', '/api/annotations/:id');
        try {
            const randomId = faker_1.faker.string.uuid();
    ...
    async updateLocation() {
        ...
        try {
            this.userProfile.location = {
                lat: this.userProfile.location.lat + faker_1.faker.number.float({ min: -0.001, max: 0.001 }),
                lng: this.userProfile.location.lng + faker_1.faker.number.float({ min: -0.001, max: 0.001 })
            };
    ...
    async uploadMedia() {
        ...
        try {
            const mockImageData = Buffer.from(faker_1.faker.lorem.paragraphs(10));
            const formData = new FormData();
            const response = await this.httpClient.post('/api/media/upload', {
                filename: faker_1.faker.system.fileName({ extensionCount: 1 }),
                mimeType: 'image/jpeg',
    ...
    async shareAnnotation() {
        ...
        try {
            const randomId = faker_1.faker.string.uuid();
            const shareData = {
                platform: faker_1.faker.helpers.arrayElement(['wechat', 'weibo', 'qq', 'email']),
    ...
    async followUser() {
        ...
        try {
            const randomUserId = faker_1.faker.string.uuid();
    ...
    async withdrawEarnings() {
        ...
        try {
            const withdrawData = {
                ...
                method: faker_1.faker.helpers.arrayElement(['alipay', 'wechat', 'bank_card'])
            };
    ...
    async viewTransactionHistory() {
        ...
        try {
            const params = {
                page: faker_1.faker.number.int({ min: 1, max: 5 }),
                limit: faker_1.faker.number.int({ min: 10, max: 50 })
            };
    ...
    async createPaidAnnotation() {
        ...
        try {
            const annotationData = {
                ...
                category: faker_1.faker.helpers.arrayElement(['industrial', 'sewage', 'garbage', 'chemical']),
                intensity: faker_1.faker.number.int({ min: 6, max: 10 }),
                tags: faker_1.faker.helpers.arrayElements(['严重污染', '紧急处理', '环境危害'], { min: 1, max: 3 }),
                paymentAmount: faker_1.faker.number.int({ min: 500, max: 2000 })
            };
    ...
    async validateAnnotation() {
        ...
        try {
            ...
            const validationData = {
                isValid: faker_1.faker.datatype.boolean({ probability: 0.8 }),
                reason: faker_1.faker.lorem.sentence(),
                confidence: faker_1.faker.number.float({ min: 0.6, max: 1.0 })
            };
    ...
    async reportSpam() {
        ...
        try {
            const reportData = {
                ...
                targetType: faker_1.faker.helpers.arrayElement(['annotation', 'comment', 'user']),
                reason: faker_1.faker.helpers.arrayElement(['spam', 'fake', 'inappropriate', 'offensive']),
    ...
    async moderateContent() {
        ...
        try {
            const moderationData = {
                contentId: faker_1.faker.string.uuid(),
                action: faker_1.faker.helpers.arrayElement(['approve', 'reject', 'flag']),
    ...
    async updateAnnotation() {
        ...
        try {
            ...
            const updateData = {
                description: faker_1.faker.lorem.paragraphs(2),
                intensity: faker_1.faker.number.int({ min: 1, max: 10 }),
                tags: faker_1.faker.helpers.arrayElements(['更新', '修正', '补充'], { min: 1, max: 2 })
            };
    ...
    createAction(actionName, endpoint) {
        return {
            timestamp: Date.now(),
            action: actionName,
            endpoint,
...
class MultiAgentSimulator extends events_1.EventEmitter {
        ...
        this.scenarios.set('full', {
            ...
            agents: [
                ...Array.from({ length: 3 }, (_, i) => ({
                    ...
                    name: `探索者${i + 1}`,
                    behavior: 'explorer',
                    intensity: faker_1.faker.helpers.arrayElement(['low', 'medium', 'high']),
                    duration: 10,
                    baseUrl: process.env.API_BASE_URL || 'http://localhost:3000'
                })),
                ...Array.from({ length: 4 }, (_, i) => ({
                    ...
                    name: `标注者${i + 1}`,
                    behavior: 'annotator',
                    intensity: faker_1.faker.helpers.arrayElement(['medium', 'high']),
                    duration: 10,
                    baseUrl: process.env.API_BASE_URL || 'http://localhost:3000'
                })),
                ...Array.from({ length: 3 }, (_, i) => ({
    ...
    groupMetricsByBehavior(metrics) {
        const groups = {};
    ...
    evaluateOutcomes(metrics, expectedOutcomes) {
        const outcomes = {};
...
<body>
    <div class="container">
        ...
        <div class="section">
            ...
            <table class="table">
                ...
                <tbody>
                    ${Object.entries(report.summary.agentsByBehavior).map(([behavior, stats]) => `
                    <tr>
        ...
        <div class="section">
            ...
            <div class="agent-details">
                ${report.agents.slice(0, 10).map((agent) => `
                <div class="agent-card">
                    ...
                    <div style="margin-top: 15px;">
                        ...
                        <ul style="margin: 10px 0; padding-left: 20px;">
                            ${agent.errors.slice(0, 3).map((error) => `<li class="error">${error}</li>`).join('')}
    ...
    printSummary(summary) {
        ...
        console.log('\n👥 用户角色分布:');
        Object.entries(summary.agentsByBehavior).forEach(([behavior, stats]) => {
...
exports.MultiAgentSimulator = MultiAgentSimulator;
exports.simulator = new MultiAgentSimulator();
if (require.main === module) {
    const scenarioName = process.argv[2] || 'smoke';
    console.log('🚀 SmellPin 多代理测试启动...');
    console.log(`📋 执行场景: ${scenarioName}`);
    exports.simulator.runScenario(scenarioName)
        .then(() => {
        console.log('\n🎉 测试完成!');
        process.exit(0);
    })
...
//# sourceMappingURL=multi-agent-simulator.js.map