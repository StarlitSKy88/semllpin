...
class TestReportGenerator {
    ...
    initializeReportDirectory() {
        ...
        subDirs.forEach(dir => {
            const dirPath = path_1.default.join(this.reportDir, dir);
    ...
    generateReport() {
        const reportPath = path_1.default.join(this.reportDir, 'e2e-test-report.html');
        const reportHtml = this.buildReportHtml();
        fs_1.default.writeFileSync(reportPath, reportHtml, 'utf8');
    ...
    buildReportHtml() {
        ...
        const passedTests = this.testSuites.reduce((sum, suite) => sum + suite.results.filter(r => r.status === 'passed').length, 0);
        const failedTests = this.testSuites.reduce((sum, suite) => sum + suite.results.filter(r => r.status === 'failed').length, 0);
        const skippedTests = this.testSuites.reduce((sum, suite) => sum + suite.results.filter(r => r.status === 'skipped').length, 0);
    ...
    generateJsonReport() {
        const jsonReport = {
            ...
            summary: {
                ...
                passedTests: this.testSuites.reduce((sum, suite) => sum + suite.results.filter(r => r.status === 'passed').length, 0),
                failedTests: this.testSuites.reduce((sum, suite) => sum + suite.results.filter(r => r.status === 'failed').length, 0),
                skippedTests: this.testSuites.reduce((sum, suite) => sum + suite.results.filter(r => r.status === 'skipped').length, 0)
            },
            performanceMetrics: this.overallMetrics,
            testSuites: this.testSuites,
            recommendations: this.generateRecommendations()
        };
        const jsonPath = path_1.default.join(this.reportDir, 'raw-data', 'test-results.json');
        fs_1.default.writeFileSync(jsonPath, JSON.stringify(jsonReport, null, 2), 'utf8');
...
${this.generateRecommendations().slice(0, 3).map(rec => `- **${rec.title}**: ${rec.description}`).join('\n')}
`;
        const summaryPath = path_1.default.join(this.reportDir, 'performance-summary.md');
        fs_1.default.writeFileSync(summaryPath, summary, 'utf8');
    ...
    getPerformanceGrade() {
        ...
        if (metrics.pageLoad.fcp < 1800)
            score += 30;
        else if (metrics.pageLoad.fcp < 3000)
            score += 20;
        else
            score += 10;
        if (metrics.pageLoad.lcp < 2500)
            score += 30;
        else if (metrics.pageLoad.lcp < 4000)
            score += 20;
        else
            score += 10;
        if (metrics.pageLoad.cls < 0.1)
            score += 20;
        else if (metrics.pageLoad.cls < 0.25)
            score += 15;
        else
            score += 5;
        if (metrics.interactions.averageResponseTime < 100)
            score += 20;
        else if (metrics.interactions.averageResponseTime < 300)
            score += 15;
        else
            score += 5;
        if (score >= 85)
