...
exports.createTestAnnotation = createTestAnnotation;
exports.createMultipleTestAnnotations = createMultipleTestAnnotations;
exports.persistTestAnnotation = persistTestAnnotation;
const index_1 = require("./index");
class AnnotationFactoryClass {
    constructor() {
        this.counter = 0;
        this.smellTypes = [
    ...
    create(overrides = {}) {
        ...
        const baseAnnotation = {
            ...
            intensity: overrides.intensity ?? Math.floor(Math.random() * 5) + 1,
            ...
            verified: overrides.verified ?? Math.random() < 0.3,
            ...
            temperature: overrides.temperature ?? Math.floor(Math.random() * 40) - 10,
    ...
    createDraftAnnotation(overrides = {}) {
        return this.create({
            status: 'draft',
            publishedAt: null,
            visibility: 'private',
    ...
    createApprovedAnnotation(overrides = {}) {
        return this.create({
            status: 'approved',
            verified: true,
    ...
    createRejectedAnnotation(overrides = {}) {
        return this.create({
            status: 'rejected',
            visibility: 'private',
            moderatedAt: new Date(),
    ...
    createFlaggedAnnotation(overrides = {}) {
        return this.create({
            status: 'flagged',
    ...
    createHighIntensityAnnotation(overrides = {}) {
        return this.create({
            intensity: 5,
            smellType: '化学品味',
    ...
    createClusteredAnnotations(centerLat, centerLng, count, radius = 0.005) {
...
exports.AnnotationFactory = new AnnotationFactoryClass();
function createTestAnnotation(overrides = {}) {
    return exports.AnnotationFactory.create(overrides);
}
function createMultipleTestAnnotations(count, overrides = {}) {
    return exports.AnnotationFactory.createMultiple(count, overrides);
}
async function persistTestAnnotation(annotationData, db) {
    if (!db) {
