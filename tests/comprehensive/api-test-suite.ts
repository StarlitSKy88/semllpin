...
// 测试服务器和数据库配置
import { createTestServer, closeTestServer } from '../setup/testServer';
  ...
  describe('1. 用户认证API测试', () => {
    ...
    describe('用户登录', () => {
      ...
      it('应该成功登录有效用户', async () => {
        ...
        // 验证JWT令牌
        const decoded = jwt.verify(response.body.data.token, process.env.JWT_SECRET || 'test-secret') as any;
      ...
      it('应该记录登录尝试', async () => {
        ...
        // 验证登录尝试是否被记录（假设有日志表）
        const attempts = await testDb.query(
  ...
  describe('2. LBS相关API测试', () => {
    ...
    describe('位置上报', () => {
      it('应该成功上报位置', async () => {
        ...
        // 验证位置数据存储
        const storedLocation = await testDb.query(
      ...
      it('应该处理高频位置更新', async () => {
        const locations = Array.from({ length: 10 }, () => 
          testDataFactory.createLocationData()
        );
    ...
    describe('地理围栏检测', () => {
      ...
      beforeEach(async () => {
        ...
        const annotation = await testDb.query(
          'INSERT INTO annotations (user_id, latitude, longitude, smell_type, intensity, description) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id',
          [testUser.id, testAnnotation.latitude, testAnnotation.longitude, testAnnotation.smellType, testAnnotation.intensity, testAnnotation.description]
        );
      ...
      it('应该检测用户进入地理围栏', async () => {
        ...
        const nearbyLocation = {
          latitude: testAnnotation.latitude + 0.001, // 约100米距离
      ...
      it('应该计算准确的距离', async () => {
        const knownDistance = 500; // 500米
        const locationWithKnownDistance = testDataFactory.createLocationAtDistance(
        ...
        if (response.body.data.geofences.length > 0) {
          ...
          expect(calculatedDistance).toBeCloseTo(knownDistance, 50); // 允许50米误差
        }
      });
        ...
        const annotations = await Promise.all(
          Array.from({ length: 3 }, async () => {
            ...
            const result = await testDb.query(
              'INSERT INTO annotations (user_id, latitude, longitude, smell_type, intensity, description) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id',
              [testUser.id, data.latitude, data.longitude, data.smellType, data.intensity, data.description]
            );
    ...
    describe('奖励计算', () => {
      ...
      beforeEach(async () => {
        ...
        const annotation = await testDb.query(
          'INSERT INTO annotations (user_id, latitude, longitude, smell_type, intensity, description, reward_amount) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id',
          [testUser.id, testAnnotation.latitude, testAnnotation.longitude, testAnnotation.smellType, testAnnotation.intensity, testAnnotation.description, 10.00]
        );
      ...
      it('应该成功发现并获得奖励', async () => {
        ...
        // 验证奖励记录
        const reward = await testDb.query(
          'SELECT * FROM user_rewards WHERE user_id = $1 AND annotation_id = $2',
          [testUser.id, testAnnotation.id]
        );
      ...
      it('应该根据距离调整奖励金额', async () => {
        // 测试不同距离的奖励计算
        const distances = [10, 50, 100, 200]; // 不同距离（米）
        const rewards = [];

        for (const distance of distances) {
          const location = testDataFactory.createLocationAtDistance(
            testAnnotation.latitude,
            testAnnotation.longitude,
            distance
          );
  ...
  describe('3. 气味标记API测试', () => {
    ...
    describe('查询标注', () => {
      ...
      it('应该支持分页查询', async () => {
        ...
        // 验证页面数据不重复
        const page1Ids = page1.body.data.annotations.map((a: any) => a.id);
        const page2Ids = page2.body.data.annotations.map((a: any) => a.id);
    ...
    describe('更新标注', () => {
      ...
      it('应该记录更新历史', async () => {
        ...
        const history = await testDb.query(
          'SELECT * FROM annotation_history WHERE annotation_id = $1',
          [testAnnotation.id]
        );
    ...
    describe('标注互动', () => {
      ...
      it('应该支持点赞标注', async () => {
        ...
        // 验证点赞记录
        const like = await testDb.query(
          'SELECT * FROM annotation_likes WHERE annotation_id = $1 AND user_id = $2',
          [testAnnotation.id, testUser.id]
        );
