...
describe('7. 安全性测试', () => {
    ...
    beforeAll(async () => {
        ...
        adminToken = jsonwebtoken_1.default.sign({ userId: adminUser.id, email: adminUser.email, role: adminUser.role }, process.env.JWT_SECRET || 'test-secret', { expiresIn: '1h' });
    });
    describe('SQL注入攻击防护', () => {
    ...
    describe('XSS攻击防护', () => {
        ...
        it('应该清理评论中的XSS', async () => {
            const annotationResponse = await (0, supertest_1.default)(app)
                .post('/api/v1/annotations')
                .set('Authorization', `Bearer ${authToken}`)
                .send({
                latitude: 31.2304,
    ...
    describe('身份验证绕过测试', () => {
        ...
        it('应该拒绝被篡改的JWT令牌', async () => {
            const validToken = jsonwebtoken_1.default.sign({ userId: testUser.id, email: testUser.email, role: 'user' }, process.env.JWT_SECRET || 'test-secret', { expiresIn: '1h' });
        ...
        it('应该拒绝使用错误密钥签名的JWT令牌', async () => {
            const tokenWithWrongKey = jsonwebtoken_1.default.sign({ userId: testUser.id, email: testUser.email, role: 'admin' }, 'wrong-secret-key', { expiresIn: '1h' });
            const response = await (0, supertest_1.default)(app)
                .get('/api/v1/users/profile/me')
        ...
        it('应该防止JWT算法混淆攻击', async () => {
            const noneAlgToken = jsonwebtoken_1.default.sign({ userId: testUser.id, email: testUser.email, role: 'admin' }, '', { algorithm: 'none' });
            const response = await (0, supertest_1.default)(app)
                .get('/api/v1/users/profile/me')
        ...
        it('应该防止会话固定攻击', async () => {
            ...
            const secondLoginResponse = await (0, supertest_1.default)(app)
                .post('/api/v1/users/login')
                .send({
                email: testUser.email,
                password: testUser.password
    ...
    describe('授权绕过测试', () => {
        it('应该防止水平权限提升', async () => {
            ...
            const otherUserToken = jsonwebtoken_1.default.sign({ userId: otherUser.id, email: otherUser.email, role: 'user' }, process.env.JWT_SECRET || 'test-secret', { expiresIn: '1h' });
            const response = await (0, supertest_1.default)(app)
    ...
    describe('信息泄露防护', () => {
        it('应该不暴露敏感错误信息', async () => {
            const errorTests = [
                () => (0, supertest_1.default)(app).get('/api/v1/nonexistent-endpoint'),
                () => (0, supertest_1.default)(app).post('/api/v1/users/login').send({}),
                () => (0, supertest_1.default)(app).get('/api/v1/users/123456').set('Authorization', 'Bearer invalid'),
            ];
        ...
        it('应该防止用户枚举攻击', async () => {
            ...
            const existentUserWrongPasswordResponse = await (0, supertest_1.default)(app)
                .post('/api/v1/users/login')
                .send({
                email: testUser.email,
                password: 'wrongpassword'
    ...
    describe('速率限制和DDoS防护', () => {
        ...
        it('应该限制注册频率', async () => {
            const registrationPromises = Array.from({ length: 10 }, (_, i) => (0, supertest_1.default)(app)
                .post('/api/v1/users/register')
                .send({
        ...
        it('应该限制API调用频率', async () => {
            const apiCalls = Array.from({ length: 100 }, () => (0, supertest_1.default)(app)
                .get('/api/v1/annotations/list')
                .set('Authorization', `Bearer ${authToken}`));
        ...
        it('应该防护暴力破解攻击', async () => {
            const bruteForceattempts = Array.from({ length: 20 }, (_, i) => (0, supertest_1.default)(app)
                .post('/api/v1/users/login')
                .send({
                email: testUser.email,
                password: `password${i}`
    ...
    describe('会话安全', () => {
        it('应该支持会话失效', async () => {
            ...
            const accessAfterLogoutResponse = await (0, supertest_1.default)(app)
                .get('/api/v1/users/profile/me')
                .set('Authorization', `Bearer ${token}`);
        ...
        it('应该检测并发会话', async () => {
            ...
            const access1 = await (0, supertest_1.default)(app)
                .get('/api/v1/users/profile/me')
                .set('Authorization', `Bearer ${token1}`);
            const access2 = await (0, supertest_1.default)(app)
                .get('/api/v1/users/profile/me')
        ...
        it('应该防护会话劫持', async () => {
            ...
            const token = loginResponse.body.data.token;
            const hijackAttempt = await (0, supertest_1.default)(app)
                .get('/api/v1/users/profile/me')
                .set('Authorization', `Bearer ${token}`)
                .set('X-Forwarded-For', '192.168.1.200');
