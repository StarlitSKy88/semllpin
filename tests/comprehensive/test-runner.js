...
class ComprehensiveTestRunner {
    constructor() {
        ...
        this.testMetrics = new test_metrics_1.TestMetrics();
        this.suites = [
            {
                name: 'API基础测试',
    ...
    executeJestTest(testFile, timeout) {
        return new Promise((resolve, reject) => {
            ...
            const jestProcess = (0, child_process_1.spawn)('npx', ['jest', ...jestArgs], {
            ...
            jestProcess.on('close', (code) => {
                try {
                    let result = {};
                    if (fs_1.default.existsSync('temp-test-results.json')) {
                        const resultData = fs_1.default.readFileSync('temp-test-results.json', 'utf8');
                        result = JSON.parse(resultData);
                        fs_1.default.unlinkSync('temp-test-results.json');
                    }
                    resolve({
                        success: code === 0,
                        exitCode: code,
    ...
    async generateComprehensiveReport() {
        ...
        const reportsDir = path_1.default.join(process.cwd(), 'tests', 'reports');
        if (!fs_1.default.existsSync(reportsDir)) {
            fs_1.default.mkdirSync(reportsDir, { recursive: true });
        }
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
        const jsonReportPath = path_1.default.join(reportsDir, `comprehensive-report-${timestamp}.json`);
    ...
    generateRecommendations(successful, failed, all) {
...
<body>
    <div class="container">
        ...
        <div class="section">
            ...
            <div class="section-content">
                ${Object.entries(report.suiteResults).map(([name, result]) => `
        ...
        const htmlPath = path_1.default.join(reportsDir, `comprehensive-report-${timestamp}.html`);
    ...
    printConsoleSummary(report) {
        ...
        if (report.summary.failedSuites > 0) {
            console.log(`\n❌ 失败的测试套件:`);
            Object.entries(report.suiteResults).forEach(([name, result]) => {
...
exports.default = ComprehensiveTestRunner;
