...
class PerformanceMetrics {
  ...
  recordApiResponse(endpoint: string, responseTime: number, status: number, payloadSize?: number) {
    this.metrics.push({
      timestamp: Date.now(),
      endpoint,
      responseTime,
  ...
  recordMemorySnapshot(label: string) {
...
describe('8. 性能和负载测试', () => {
  ...
  describe('并发请求测试', () => {
    ...
    it('应该处理并发的标注创建请求', async () => {
      ...
      expect(successfulCreations.length).toBeGreaterThan(concurrentRequests * 0.8); // 至少80%成功
  ...
  describe('数据库性能测试', () => {
    it('应该高效处理大量数据插入', async () => {
      ...
      expect(successCount).toBeGreaterThan(batchSize * 0.9); // 90%成功率

      // 计算每个请求的平均时间
      const avgRequestTime = totalTime / batchSize;
      expect(avgRequestTime).toBeLessThan(50); // 平均每个请求50ms以内
    });
  ...
  describe('内存和资源使用测试', () => {
    ...
    it('应该高效管理CPU使用', async () => {
      ...
      const promises = Array.from({ length: cpuIntensiveOperations }, () => 
        request(app)
          .get('/api/v1/annotations/nearby')
          .query({
            latitude: 31.2304,
  ...
  describe('负载测试', () => {
    it('应该承受持续的中等负载', async () => {
      ...
      await new Promise(resolve => {
        setTimeout(resolve, duration + 5000); // 额外5秒等待请求完成
      });
    ...
    it('应该处理突发流量', async () => {
      ...
      expect(successCount).toBeGreaterThan(burstSize * 0.9); // 90%成功率
    });
    ...
    it('应该正确处理资源耗尽情况', async () => {
      ...
      const promises = Array.from({ length: overloadRequests }, () => 
        request(app)
          .post('/api/v1/annotations')
          .set('Authorization', `Bearer ${authToken}`)
          .send({
      ...
      // 系统应该优雅地处理过载
      expect(serverErrorCount).toBeLessThan(overloadRequests * 0.1); // 服务器错误少于10%
      expect(successCount + rateLimitCount).toBeGreaterThan(overloadRequests * 0.8); // 大部分请求被正确处理或限制
    });
  });
  ...
  describe('AutoCannon负载测试', () => {
    it('应该通过AutoCannon基准测试', async () => {
      ...
      expect(result.requests.average).toBeGreaterThan(50); // 每秒至少50个请求
    }, 15000);
    ...
    it('应该在混合负载下保持性能', async () => {
      ...
      expect(result.errors).toBeLessThan(result.requests.total * 0.01); // 错误率小于1%
      expect(result['2xx']).toBeGreaterThan(result.requests.total * 0.95); // 成功率大于95%
      expect(result.latency.p99).toBeLessThan(1000); // 99%请求在1秒内完成
    }, 20000);
  });
});