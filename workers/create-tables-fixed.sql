...
-- 创建索引
CREATE INDEX IF NOT EXISTS idx_annotations_location ON annotations(latitude, longitude);
...
CREATE INDEX IF NOT EXISTS idx_lbs_rewards_annotation_id ON lbs_rewards(annotation_id);
...
-- 插入示例管理员用户（可选）
-- INSERT INTO users (email, username, display_name, role, status, email_verified)
-- VALUES ('admin@smellpin.com', 'admin', 'SmellPin Admin', 'admin', 'active', true)
-- ON CONFLICT (email) DO NOTHING;

-- 脚本执行完成提示
SELECT 'SmellPin数据库表创建完成！所有表、索引、RLS策略和权限已设置。' as message;